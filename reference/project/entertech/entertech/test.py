...
from tkinter import (
    ...
    Event, filedialog, Entry, Toplevel
)
...
class Demo:
    ...
    def __init__(self) -> None:
        ...
        self.data = {'eeg_time': [], 'psg_time': [], 'hr_time': [], 'eegl_wave': [], 'eegr_wave': [], 'eeg_alpha_power': [], 'eeg_beta_power': [], 'eeg_theta_power': [], 'eeg_delta_power': [], 'eeg_gamma_power': [], 'eeg_quality': [], 'hr': [], 'hrv': []}
        self.services = []
        self.lock = asyncio.Lock()
        ...
        self.tk.geometry("{}x{}+{}+{}".format(self.screen_w, 838, x, y))
        self.tk.title("Demo")
        ...
        controller_frame.place(x=5, y=self.screen_h-275, width=self.screen_w - 10, height=325)
        ...
        file_frame = Frame(controller_frame, borderwidth=1, relief=SUNKEN)
        file_frame.grid(row=5, column=0)
        file_frame.place(x=5, y=255, width=self.screen_w - 10, height=60)

        file_label = Label(file_frame, text="数据存储")
        file_label.grid(row=0, column=0)

        self.store_file_btn = Button(file_frame, text="存储")
        self.store_file_btn.grid(row=1, column=0)
        self.store_file_btn.bind("<Button-1>", self.set_file_name)
        # self.store_file_btn.config(state=DISABLED)
        self.store_file_btn = Button(file_frame, text="记录时间戳")
        self.store_file_btn.grid(row=1, column=1)
        self.store_file_btn.bind("<Button-1>", self.record_timestamp)
    ...
    async def wear_status_callback(self, wear_status: bool) -> None:
        ...
        # print("Wear status: {}".format(wear_status))
    ...
    async def base_service_subscribe(self, data: str) -> None:
        ...
        if data.data:
            for key, values in data.data.items():
                keys = key.split('_')
                if keys[0] == 'eeg':
                    await asyncio.gather(self.update_eeg_data(time_now, values))
                elif keys[0] == 'hr-v2':
                    await asyncio.gather(self.update_hr_data(time_now, values))
    ...
    async def update_eeg_data(self, time, value) -> None:
        async with self.lock:
            self.data['eeg_time'].extend([time for i in range(len(value['eegl_wave']))])
            self.data['psg_time'].append(time)
            self.data['eegl_wave'].extend(value['eegl_wave'])
            self.data['eegr_wave'].extend(value['eegr_wave'])
            self.data['eeg_alpha_power'].append(value['eeg_alpha_power'])
            self.data['eeg_beta_power'].append(value['eeg_beta_power'])
            self.data['eeg_theta_power'].append(value['eeg_theta_power'])
            self.data['eeg_delta_power'].append(value['eeg_delta_power'])
            self.data['eeg_gamma_power'].append(value['eeg_gamma_power'])
            self.data['eeg_quality'].append(value['eeg_quality'])

    async def update_hr_data(self, time, value) -> None:
        async with self.lock:
            self.data['hr_time'].append(time)
            self.data['hr'].append(value['hr'])
            self.data['hrv'].append(value['hrv'])

    def store_file(self, event: Event) -> None:
        """存储文件
        """
        if self.widget_disabled(event.widget):
            return
        ...
        self.write_log(f"{file_path}")


        time_start = str(self.time_start)
        df_eeg = pd.DataFrame({'time': self.data['eeg_time'], 'Fp1': self.data['eegl_wave'], 'Fp2': self.data['eegr_wave']})
        df_psg = pd.DataFrame({'time': self.data['psg_time'], 'alpha': self.data['eeg_alpha_power'], 'beta': self.data['eeg_beta_power'], 'theta': self.data['eeg_theta_power'], 'delta': self.data['eeg_delta_power'], 'gamma': self.data['eeg_gamma_power'], 'quality': self.data['eeg_quality']})
        df_hr = pd.DataFrame({'time': self.data['hr_time'], 'hr': self.data['hr'], 'hrv': self.data['hrv']})

        if file_path:
            if self.entry.get() != '':
                file_path = os.path.join(file_path, self.entry.get())
            else:
                file_path = os.path.join(file_path, 'default')
            os.makedirs(file_path, exist_ok=True)
            try:
                with open(os.path.join(file_path, 'time_start.txt'),'w') as file:
                    file.write(time_start)
                df_eeg.to_csv(os.path.join(file_path, 'eeg.csv'), index=False)
                df_psg.to_csv(os.path.join(file_path, 'psg.csv'), index=False)
                df_hr.to_csv(os.path.join(file_path, 'hr.csv'), index=False)
                # 文件保存成功
                print("文件保存成功！")
                self.write_log("File saved.")
                self.store_window.destroy()
            except Exception as e:
                # 文件保存失败
                print("文件保存失败:", e)
                self.write_log("File save failed.")
        else:
            # 用户取消了保存操作
            print("保存操作已取消。")
            self.write_log("File save canceled.")
        
    def set_file_name(self, event: Event) -> None:
        """设置文件名
        """
        if self.widget_disabled(event.widget):
            return
        self.store_window = Toplevel()
        # 设置在屏幕中的位置为屏幕中央

        self.store_window.title("实验数据存储")
        w, h = self.tk.maxsize()
        x, y = (w - self.screen_w)//2, (h - self.screen_h)//2
        self.store_window.geometry(f"160x115+{x+400}+{y+300}")  # Set the size of the new window to 400x300

        label = Label(self.store_window, text="请输入受试者编号：")
        label.grid(row=0, column=0)
        label.place(x=5, y=5, width=150, height=20)
        
        self.entry = Entry(self.store_window)
        self.entry.grid(row=1, column=0)
        self.entry.place(x=5, y=30, width=150, height=40)

        # 在新窗口中添加按钮
        button1 = Button(self.store_window, text="保存")
        button1.grid(row=2, column=0)
        button1.place(x=10, y=75, width=65, height=30)
        button1.bind("<Button-1>", self.store_file)
        
        button2 = Button(self.store_window, text="取消", command=self.store_window.destroy)
        button2.grid(row=2, column=1)
        button2.place(x=85, y=75, width=65, height=30)

    def record_timestamp(self, event: Event) -> None:
        self.time_start = time.time()
        self.write_log('record time:'+str(self.time_start))
